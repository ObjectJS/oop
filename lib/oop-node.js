...
classmethod.setTo = function(cls, name, member) {
	...
	cls[name] = proto[name] = function() {
		...
		args.unshift(im_self);
		return member.im_func.apply(im_self.__this__, args);
	};
};
...
Type.__setattr__ = function(cls, name, member) {
	...
	// property/classmethod/staticmethod
		...
		member.__class__.setTo(cls, name, member);
	}
	// this.a = someObject
	else {
		proto[name] = member;
...
function Class() {
	...
	// 创建&初始化
	var cls = Type.__new__(Type, name, base, dict);
