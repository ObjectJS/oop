...
classmethod.setTo = function(cls, name, member) {
	...
	function _classmethod() {
		...
		args.unshift(im_self);
		return func.apply(im_self.__this__, args);
	}
	func.__name__ = name;
...
function parent() {
	var ownCls = this.cls; // 拥有此方法的代码书写的类
...
Function.setTo = Type.setTo = function(cls, name, member) {
	cls[name] = cls.prototype[name] = member;
};

Function.initialize = Type.initialize = function() {
...
Function.__new__ = Type.__new__ = function(metaclass, name, base, dict) {
	var cls = function() {
		this.__class__ = cls;
...
Function.__setattr__ = function(cls, name, member) {
	var proto = cls.prototype;
	var memberType;
